...
function Ut(t, e, r, i, s) {
    let o, a, n = 0;
    do a = e + (r - e) / 2, o = At(a, i, s) - t, o > 0 ? r = a : e = a; while (Math.abs(o) > jt && ++n < Bt);
    return a
}
...
var C = class {
    constructor(e, r = [0, 1], {
        ...
        if (this.startTime = null, this.rate = 1, this.t = 0, this.cancelTimestamp = null, this.easing = g, this.duration = 0, this.totalDuration = 0, this.repeat = 0, this.playState = "idle", this.finished = new Promise((h, m) => {
        ...
        this.tick = h => {
            ...
            o = o;
            let y = 0;
            this.pauseTime !== void 0 ? y = this.pauseTime : y = (h - this.startTime) * this.rate, this.t = y, y /= 1e3, y = Math.max(y - o, 0), this.playState === "finished" && this.pauseTime === void 0 && (y = this.totalDuration);
            let x = y / this.duration,
                z = Math.floor(x),
                E = x % 1;
            !E && x >= 1 && (E = 1), E === 1 && z--;
            let X = z % 2;
            (c === "reverse" || c === "alternate" && X || c === "alternate-reverse" && !X) && (E = 1 - E);
            let j = y >= this.totalDuration ? 1 : Math.min(E, 1),
                M = p(this.easing(j));
            e(M), this.pauseTime === void 0 && (this.playState === "finished" || y >= this.totalDuration + a) ? (this.playState = "finished", (m = this.resolve) === null || m === void 0 || m.call(this, M)) : this.playState !== "idle" && (this.frameRequestId = requestAnimationFrame(this.tick))
        }, this.play()
    }
    play() {
...
function Pt(t, e, r, i = {}, s) {
    ...
    let A = K(e),
        Y = bt(X.values, A),
        O = F.get(A);
    return rt(Y.animation, !(V(m) && Y.generator) && i.record !== !1), () => {
        let Z = () => {
                var l, B;
                return (B = (l = G.get(t, A)) !== null && l !== void 0 ? l : O == null ? void 0 : O.initialValue) !== null && B !== void 0 ? B : 0
            },
            u = Dt(Rt(r), Z),
            gt = It(u, O);
        if (V(m)) {
        ...
        if (k(A) && (D.cssRegisterProperty() ? wt(A) : M = !1), j && !D.linearEasing() && (v(m) || b(m) && m.some(v)) && (M = !1), M) {
            O && (u = u.map(R => S(R) ? O.toDefaultUnit(R) : R)), u.length === 1 && (!D.partialKeyframes() || a) && u.unshift(Z());
            let l = {
                delay: w.ms(c),
                duration: w.ms(f),
                endDelay: w.ms(p),
            ...
            n = t.animate({
                [A]: u,
                offset: z,
                easing: b(m) ? m.map(R => dt(R, f)) : void 0
            }, l), n.finished || (n.finished = new Promise((R, zt) => {
                n.onfinish = R, n.oncancel = zt
            }));
            let B = u[u.length - 1];
            n.finished.then(() => {
                y || (G.set(t, A, B), n.cancel())
            }).catch(_), E || (n.playbackRate = 1.000001)
        } else if (s && j) u = u.map(l => typeof l == "string" ? parseFloat(l) : l), u.length === 1 && u.unshift(parseFloat(Z())), n = new s(l => {
            G.set(t, A, gt ? gt(l) : l)
...
var Jt = t => t(),
    W = (t, e, r = d.duration) => new Proxy({
        animations: t.map(Jt).filter(Boolean),
        duration: r,
        options: e
    }, te),
    Qt = t => t.animations[0],
    te = {
        get: (t, e) => {
            ...
            switch (e) {
                ...
                case "finished":
                    return t.finished || (t.finished = Promise.all(t.animations.map(ee)).catch(_)), t.finished;
                case "stop":
                    return () => {
                ...
                default:
                    return typeof(r == null ? void 0 : r[e]) == "undefined" ? void 0 : () => t.animations.forEach(i => i[e]())
            }
        },
        set: (t, e, r) => {
            switch (e) {
...
var T = ({
    ...
    let r = (o, a = {}) => {
        ...
        Array.isArray(o) || (o = [o]);
        for (let p of o) p.style.display = c, Object.assign(p.style, t), n && f !== void 0 ? f ? n.insertBefore(p, f.nextSibling) : n.prepend(p) : n && n.appendChild(p)
    };
    return {
        prepareIn: r,
        animateIn: async (o, a = {}) => {
        ...
        animateOut: async (o, a = {}) => {
            ...
            if (Array.isArray(o) || (o = [o]), o = o.filter(x => document.body.contains(x)), !o.length) return;
            let {
                finished: y
            } = ot(o, e, { ...m,
                delay: f ? it(f) : void 0,
            ...
            await y;
            for (let x of o) c && p !== void 0 ? p ? c.insertBefore(x, p.nextSibling) : c.prepend(x) : c && c.appendChild(x), n ? x.remove() : x.style.display = h
        }
    }
};
...
export {
    Fi as animations, wi as easings
};